...
Page({
  ...
  getKey(){//点击获取验证码按钮时触发
    if(this.data.email.match(/(^[a-z])[0-9]{9,9}((@hust.edu.cn)$)/)) {//判断格式是否正确
      this.setData({firstEmail: this.data.email,emailTrue: true,})//设置firstEmail;
      wx.request({
        url: 'url',//请求服务器发送验证码，url暂定
        header: {
          "content-type": 'application/json',
        },
        method: 'POST',
        data:{
          email: this.data.email,//传入邮箱
        },
        success(res){
          this.stepTwo(res);//成功获得响应会进行下一步
        }
      })
    } else {
      this.setData({
  ...
  stepTwo(res) {//成功发送验证码后的页面变化
    ...
    let interval=setInterval(()=>{
      this.setData({time:i});//设置60间隔按钮内容
      i--;
      if(i===0) {//60s结束时
        this.setData({
  ...
  keyJudge() {//判断验证码格式
    if(this.data.keyValue==""||this.data.keyValue.length===6) {//为空或者位数正确
      this.setData({keyTrue: true,})
    } else {
      this.setData({keyTrue:false,})//边框变红
    }
    if(this.data.keyValue.length===6) {//如果输入了且长度为6
      this.setData({loginShow: true,timeIntervalShow: false,})//显示登录按钮隐藏其他按钮
    } else {
      this.setData(
        {
          loginShow: false,
  ...
  login() {
    this.setData({secondEmail: this.data.email});
